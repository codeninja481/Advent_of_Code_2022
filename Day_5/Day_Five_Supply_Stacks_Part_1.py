...
stored in stacks of marked crates, but because the needed supplies are buried under many other crates, 
...
bottom, and 
  crate N is on top. Stack 2 contains three crates; from bottom to top, they are crates M, C, and D. 
...
[Z] [M] [P]
 1   2   3 
In the second step, three crates are moved from stack 1 to stack 3. Crates are moved one at a time, so 
the first crate to be moved (D) ends up below the second and third crates:
...
"""
#Stacks = open("Day_Five_Supply_Stacks_Input.txt").read().split('\n')
Stacks = open("Day_Five_Supply_Stacks_Test_Input.txt").read().split('\n')
